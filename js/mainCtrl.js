angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = { 
      ...
      columnDefs: [
        {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}"><img src="https://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getData = function(artist) {
      itunesService.getData(artist).then(function(response) {
